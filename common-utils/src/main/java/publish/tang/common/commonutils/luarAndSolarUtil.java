...
/**
 * Created by tanghongbin on 2016/11/18.
 */

public class luarAndSolarUtil {
         ...
         * 将农历日期转换为公历日期
         */
        public static final String lunarToSolar(int year, int month, int monthDay) {
            int dayOffset;
            int leapMonth;
            int i;
            boolean isLeapMonth = judgeIsLeapMonth(year,month);
        ...
        //判断是否是闰月
        public static boolean judgeIsLeapMonth(int year, int month) {
            if (year % 4 != 0) {
                return false;
            }
            if (month == 2) {
